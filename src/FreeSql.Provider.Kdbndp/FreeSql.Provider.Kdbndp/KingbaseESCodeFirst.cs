...
namespace FreeSql.Kdbndp
{
    ...
    class KingbaseESCodeFirst : Internal.CommonProvider.CodeFirstProvider
    {
        ...
        protected override string GetComparisonDDLStatements(params TypeSchemaAndName[] objects)
        {
            ...
            var public_ = searchPath;// _isSysV8R3 == true ? searchPath : "public";
            ...
            foreach (var obj in objects)
            {
                ...
                if (_orm.Ado.ExecuteScalar(CommandType.Text, string.Format($" select 1 from {pg_}tables a inner join {pg_}namespace b on b.nspname = a.schemaname where b.nspname ='{{0}}' && a.tablename = '{{1}}'", tbname)) == null)
                { //表不存在
                    if (tboldname != null)
                    {
...
and upper(text(b.nspname || '.' || a.relname)) not in ('PUBLIC.GEOGRAPHY_COLUMNS','PUBLIC.GEOMETRY_COLUMNS','PUBLIC.RASTER_COLUMNS','PUBLIC.RASTER_OVERVIEWS')", tbname[0], tbname[1])));
